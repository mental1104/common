...
func TestProducerAsyncCallbacksSuccessAndFailureOnce(t *testing.T) {
	for _, sendErr := range []error{nil, errors.New("broker unavailable")} {
		...
		got := <-done
		if (sendErr == nil) != (got == nil) {
			t.Fatalf("sendErr=%v callbackErr=%v", sendErr, got)
		}
		...
		if count.Load() != 1 || w.closed.Load() != 1 {
			t.Fatalf("callback=%d close=%d", count.Load(), w.closed.Load())
		}
		if !errors.Is(producer.Send(context.Background(), "late"), commonmq.ErrClosed) {
...
func TestProducerCallbackMayCloseProducer(t *testing.T) {
	...
	if w.closed.Load() != 1 {
		t.Fatalf("close count=%d", w.closed.Load())
	}
}
...
func TestConsumerReceiveAckNackAndTimeout(t *testing.T) {
	...
	if first.commits.Load() != 1 || first.closed.Load() != 1 {
		t.Fatal("ack/nack not forwarded")
	}
	message, err = consumer.Receive(context.Background(), time.Second)
	if err != nil || string(message.Payload) != "two" {
		t.Fatalf("payload=%q err=%v", message.Payload, err)
	}
...
func TestConsumerListenerMayAcknowledge(t *testing.T) {
	r := &fakeReader{message: kafkago.Message{Value: []byte("one")}}
	q := newMessageQueue(nil, func(string, string) (reader, error) { return r, nil })
	...
	if r.commits.Load() != 1 {
		t.Fatalf("commit count=%d", r.commits.Load())
	}
}
