...
func (p *fakeProducer) Send(context.Context, *pulsargo.ProducerMessage) (pulsargo.MessageID, error) {
	if p.sendErr != nil { return nil, p.sendErr }
...
func (c *fakeConsumer) Receive(context.Context) (pulsargo.Message, error) { return c.message, c.receiveErr }
...
func (c *fakeClient) Subscribe(pulsargo.ConsumerOptions) (consumerClient, error) {
	...
	if index >= len(c.consumers) { return nil, errors.New("no consumer") }
...
func TestProducerAsyncCallbacksSuccessAndFailureOnce(t *testing.T) {
	for _, sendErr := range []error{nil, errors.New("broker unavailable")} {
		...
		if err := producer.SendAsync(context.Background(), "message", func(r commonmq.SendResult) { count.Add(1); done <- r }); err != nil { t.Fatal(err) }
		got := <-done
		if (sendErr == nil) != (got.Err == nil) { t.Fatalf("sendErr=%v callback=%+v", sendErr, got) }
		if sendErr == nil && got.MessageID == "" { t.Fatal("successful send should expose message id") }
		if err := producer.Close(); err != nil { t.Fatal(err) }
		if err := producer.Close(); err != nil { t.Fatal(err) }
		if count.Load() != 1 || native.closed.Load() != 1 || native.flushes.Load() != 1 { t.Fatal("callback/close must be exactly once") }
	}
}
...
func TestConsumerAckNackUnsubscribeResubscribeAndTimeout(t *testing.T) {
	...
	if err := consumer.Unsubscribe(context.Background()); err != nil { t.Fatal(err) }
	if err := consumer.Resubscribe(context.Background()); err != nil { t.Fatal(err) }
	if first.acks.Load() != 1 || first.nacks.Load() != 1 || first.unsubscribes.Load() != 1 { t.Fatal("consumer operations were not forwarded") }
