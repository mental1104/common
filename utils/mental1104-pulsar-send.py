...
from pulsar.schema import AvroSchema, JsonSchema, BytesSchema
from mental1104.connector.pulsar import PulsarEnvironment
    ...
    def _load_schema(self, schema_path: str):
        ...
        if os.path.isdir(schema_path):
            ...
            if not os.path.exists(schema_file):
                ...
                return BytesSchema() # 如果目录下没有找到该文件，退出
        ...
        msg_type = schema_dict.get('type')
        if msg_type == 'record':
            schema = AvroSchema(None, schema_dict)
        else:
            schema = BytesSchema()
        # 使用 Pulsar 的 AvroSchema 来处理 JSON schema
        return schema

    def send_message(self, message_data: bytes, data_file: str):
        """
        向指定 Topic 发送消息。
        ...
        """
        try:
            data_to_send = message_data
            if (isinstance(self.schema, AvroSchema) or isinstance(self.schema, JsonSchema)) and data_file.endswith('.json'):
                data_to_send = json.loads(message_data.decode('utf-8'))
            self.producer.send(data_to_send)
            print(f"Message sent successfully: {data_to_send}")
...
def parse_arguments():
    ...
    # Pulsar 服务地址
    parser.add_argument('--pulsar-url', type=str, help=f"Pulsar 服务主机地址（不包含协议和端口，例如：localhost）。可以通过环境变量 {PulsarEnvironment.PULSAR_HOST.value} 来设置。")
    # Pulsar 服务端口
    parser.add_argument('--pulsar-port', type=int, help=f"Pulsar 服务生产者端口（例如：6650）。可以通过环境变量 {PulsarEnvironment.PULSAR_BROKER_PORT.value} 来设置。")
    # 完整主题名称
    ...
    # Pulsar 认证 Token
    parser.add_argument('--token', type=str, help=f"Pulsar 认证 Token（可选）。可以通过环境变量 {PulsarEnvironment.PULSAR_TOKEN.value} 来设置。")
...
def get_pulsar_service(cli_url, cli_producer_port):
    ...
    :return: 主机地址和生产者端口。
    """
    pulsar_url = cli_url or os.getenv(PulsarEnvironment.PULSAR_HOST.value)
    producer_port = cli_producer_port or os.getenv(PulsarEnvironment.PULSAR_BROKER_PORT.value)
...
def get_pulsar_token(cli_token):
    ...
    :return: Pulsar Token。
    """
    return cli_token or os.getenv(PulsarEnvironment.PULSAR_TOKEN.value)
...
def process_data_path(data_path, full_topic):
    """
    ...
    if os.path.isdir(data_path):
        ...
        files = [os.path.join(data_path, f) for f in os.listdir(data_path) if f == f"{get_last_part(full_topic)}.json"]
        if not files:
...
def main():
    ...
    data_files = process_data_path(data_path, args.full_topic)
...
if __name__ == '__main__':
    # 使用示例：./send_pulsar_data --pulsar-url 192.168.31.239 --pulsar-port 6650 --full-topic test_yzf/test/test_topic --schema-path /home/espeon/code/pulsar_schema --data-path /home/espeon/code/pulsar_data
    main()
