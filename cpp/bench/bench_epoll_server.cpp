...
static void BM_Epoll_Pipe_Read_1B(benchmark::State &state) {
  ...
  srv.add_fd(rfd, EPOLLIN, [&](int fd) {
    ...
    for (;;) {
      ...
      if (n > 0)
        bytes.fetch_add((size_t)n, std::memory_order_relaxed);
      else
        break; // EAGAIN or 0
    }
  });
...
static bool ensure_fd_budget_or_skip(benchmark::State &state,
                                     size_t pipe_count) {
  constexpr size_t EXTRA = 128; // kqueue/epoll + benchmark overhead
  size_t desired = pipe_count * 2 + EXTRA;
  if (ensure_fd_limit(desired))
    return true;
  std::cerr << "[bench] insufficient RLIMIT_NOFILE (need >= " << desired
            << " fds)\n";
...
static bool make_pipes(size_t N, std::vector<int> &rfds,
                       std::vector<int> &wfds) {
  rfds.resize(N);
  wfds.resize(N);
...
static void BM_Epoll_ManyFds_Sparse(benchmark::State &state) {
  ...
  if (!ensure_fd_budget_or_skip(state, N))
    return;
  std::vector<int> rfds, wfds;
  if (!make_pipes(N, rfds, wfds)) {
    state.SkipWithError("pipe() failed");
    return;
  }
...
OUT:
  ...
  for (size_t i = 0; i < N; ++i)
    srv.remove_fd(rfds[i]);
  close_pipes(rfds, wfds);
}
BENCHMARK(BM_Epoll_ManyFds_Sparse)
    ->Args({1024, 8})
    ->Args({4096, 8})
    ->Args({8192, 8});
...
BENCHMARK(BM_Poll_ManyFds_Sparse)
    ->Args({1024, 8})
    ->Args({4096, 8})
    ->Args({8192, 8});
