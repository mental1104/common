...
std::string make_complex_json(std::size_t users,
                              std::size_t scores_len = 16,
                              std::size_t matrix_n   = 16,
                              std::size_t tags       = 12) {
    std::string s;
    s.reserve(users * (256 + scores_len * 6) + matrix_n * matrix_n * 3 + tags * 16 + 4096);

    s += R"({"meta":{"version":1,"title":"Benchmark Fixture","active":true,"ratio":0.12345,"text":"你好, 😊","tags":[)";
    ...
    for (size_t i = 0; i < users; ++i) {
        ...
        s += R"(","flags":[true,false,null],"address":{"city":"Shenzhen","zip":518000,"lines":["Apt 1","Street 2"]}})";
        s += R"(,"created_at":")";
...
// 标签类型：在编译期选择不同枚举值
struct UseCjson     { static constexpr auto parser = mental1104::JsonParser::CJSON; };
struct UseRapidjson { static constexpr auto parser = mental1104::JsonParser::RapidJSON; };

// gbench 入口
static void BM_iface_cjson(benchmark::State& s)  { BM_iface_parse(s, UseCjson{}); }
static void BM_iface_rapid(benchmark::State& s)  { BM_iface_parse(s, UseRapidjson{}); }
