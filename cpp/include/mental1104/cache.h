...
#include <limits>       // 新增：std::numeric_limits
#include <type_traits>  // 新增：std::decay_t
#include <utility>      // 新增：std::forward
...
auto make_cache(F &&func) {
  ...
  const size_t unlimited_capacity =
      std::numeric_limits<size_t>::max(); // 最大容量
