...
// 统一的调度器创建器：BasicCoroutineScheduler 需要 IExecutor&，AsyncSimpleCoroutineScheduler 需要 async_simple::Executor
...
#if defined(M1104_HAS_ASYNC_SIMPLE)
  else if constexpr (std::is_constructible_v<Scheduler,
                                             std::shared_ptr<async_simple::Executor>>) {
    static_assert(std::is_base_of_v<async_simple::Executor, ExecutorAdapter>,
                  "ExecutorAdapter must derive async_simple::Executor for this scheduler");
    return Scheduler(std::static_pointer_cast<async_simple::Executor>(exec_ptr));
  }
#endif
  else {
...
// ExecutorAdapter: 把底层线程池封装到内部、实现 IExecutor（以及可选的 async_simple::Executor）
// Scheduler: 协程调度器类型，需实现 ICoroutineScheduler 接口
template <class ExecutorAdapter,
