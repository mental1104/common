...
class ContainerPrinter {
   private:
    ...
    static typename std::enable_if<
        ...
        void>::type
    print_size(const Container& c, std::ostream& out) {
        out << "(size: " << std::distance(c.begin(), c.end()) << ") "
            << std::endl;
    }
#endif
    ...
    static void print_info(const Container& c, bool show_info,
        ...
        if constexpr (is_map<V>::value) {  // If value is map or unordered_map,
            ...
            for (const auto& [nested_key, nested_value] : value) {
                ContainerPrinter::print_map_or_unordered_map(
                    nested_key, nested_value, first, indent_level + 1, out,
                    loc);
                first = false;
            }
    ...
    static void print_internal(const std::map<K, V>& m, bool show_info,
        ...
        for (const auto& [key, value] : m) {
            ContainerPrinter::print_map_or_unordered_map(key, value, first, 1,
                                                         out, loc);
            first = false;
        }
        out << "\n}" << std::endl;
    ...
    // Print unordered_map
    template <typename K, typename V, typename SourceLocation = void>
    static void print_internal(const std::unordered_map<K, V>& m,
                               bool show_info, std::ostream& out,
                               SourceLocation loc = {}) {
        ContainerPrinter::print_info(m, show_info, out, loc);
        out << "{\n";
        bool first = true;
...
#if __cplusplus >= 202002L
    static void print(
        const Container& c, std::ostream& out = std::cout,
        bool show_info = true,
