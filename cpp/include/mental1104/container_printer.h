...
private:
  ...
  static typename std::enable_if<
      !std::is_same<Container,
                    std::forward_list<typename Container::value_type>>::value,
      void>::type
  ...
  static void print_info(const Container &c, bool show_info, std::ostream &out,
    ...
    if (show_info) {
      out << "[File: " << loc.file_name() << ", Line: " << loc.line()
          << "] "; // Use file_name() to get file path
  ...
  template <typename Container, typename SourceLocation = void>
  static void print_internal(const Container &c, bool show_info,
  ...
  static void print_map_or_unordered_map(const K &key, const V &value,
    ...
    if constexpr (is_map<V>::value) { // If value is map or unordered_map,
                                      // recursively process
      out << "{\n";
      bool first = true;
      for (const auto &[nested_key, nested_value] : value) {
        ContainerPrinter::print_map_or_unordered_map(
            nested_key, nested_value, first, indent_level + 1, out, loc);
        first = false;
      }
  ...
  // Print map
  template <typename K, typename V, typename SourceLocation = void>
  static void print_internal(const std::map<K, V> &m, bool show_info,
                             std::ostream &out, SourceLocation loc = {}) {
    ContainerPrinter::print_info(m, show_info, out, loc);
    out << "{\n";
    bool first = true;
    for (const auto &[key, value] : m) {
      ContainerPrinter::print_map_or_unordered_map(key, value, first, 1, out,
                                                   loc);
      first = false;
    }
    out << "\n}" << std::endl;
  ...
  // Print unordered_map
  template <typename K, typename V, typename SourceLocation = void>
  static void print_internal(const std::unordered_map<K, V> &m, bool show_info,
                             std::ostream &out, SourceLocation loc = {}) {
    ContainerPrinter::print_info(m, show_info, out, loc);
    out << "{\n";
    bool first = true;
  ...
  static void print_internal(const LRUCache<Ret, Args...> &cache,
                             std::ostream &os) {
    os << "{\n";
  ...
  template <typename Ret, typename... Args>
  static void print_internal(const LFUCache<Ret, Args...> &cache,
                             std::ostream &os) {
    os << "{\n";
...
public:
  ...
  template <typename Ret, typename... Args>
  static void print(LRUCache<Ret, Args...> &&cache,
  ...
  // 打印 LFUCache
  template <typename Ret, typename... Args>
  static void print(LFUCache<Ret, Args...> &&cache,
                    std::ostream &os = std::cout) {
