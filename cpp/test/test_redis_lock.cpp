...
// - MultiThreadStringIncrementTest：持锁下读取/累加 Redis 中的字符串值，最终值等于并发数×循环次数。
// - LockExpiresAndReacquirable：等待锁自然过期后可再次获取。
// - 测试依赖环境变量 REDIS_HOST/REDIS_PORT/REDISCLI_AUTH 创建连接，连接失败会跳过。
...
TEST_F(
    RedisLockTest,
    MultiThreadMutexTest) { // TEST_F 按测试夹具类分组，自动复用 SetUp/成员；TEST 按手动命名的套件分组
  // 定义测试中使用的锁键，并确保测试前删除已有的key
  std::string lock_key =
      mental1104::key_with_random_suffix("test:distributed_lock"); // gtest 默认串行执行套件；若并行跑多实例/多进程共用同一 Redis，可能与其它进程同名 key 竞争，可加前缀/随机后缀避冲突
  try {
    redis_->del({lock_key}); // 返回 long long: 删除的键数量，存在为 1，不存在为 0，错误抛异常
  } catch (const Error &err) {
    std::cerr << "Error cleaning up key: " << err.what() << std::endl;
  }

  const int thread_count = 100;
  std::atomic<int> counter(0); // 原子读写避免数据竞争；此处只用 load/store 单步操作，不需要额外锁
  std::vector<std::thread> threads;

  // 每个线程争抢锁，成功后对共享计数器做加1操作
  auto thread_func = [&counter, &lock_key]() { // redis_ 静态共享连接；lock_key 按引用共享同一字符串（其作用域覆盖所有线程生命周期）；counter 用引用以原子共享计数
    ...
    while (true) {
      if (lock.try_lock(std::chrono::milliseconds(5000))) {
        auto critical = [&]() {
          ...
          mental1104::sleep_for(std::chrono::milliseconds(50)); // 模拟工作耗时
  ...
  std::for_each(threads.begin(), threads.end(), [](std::thread &t) {
    // joinable() 仅表示未 join/detach，线程跑完但未回收也为 true；按序 join 阻塞主线程，其他线程仍并发。需要自动回收可用 C++20 std::jthread
    if (t.joinable())
      t.join();
  });
  // 等价的传统写法：
  ...
  EXPECT_EQ(counter.load(),
      ...
      << " expected thread_count=" << thread_count
      << " (lock_key=" << lock_key << ")"; // EXPECT_EQ(expected, actual) 固定两个参数，前者期望值，后者实际值；额外提示可用 << 拼接到失败消息（写入 gtest 的输出流）
}
...
TEST_F(RedisLockTest, MultiThreadStringIncrementTest) { // 同上
  ...
  // 2) 外层循环 loop_count 次；每次都在内层 while 里反复 try_lock(5s) 直到成功。
  // 3) 持锁后读取 array_key 的值；如果键不存在，解锁并跳出本轮。
  // 4) 打印线程号和当前值，转成 int 自增，再写回 Redis。
  // 5) 解锁，结束本次循环；未取到锁则打印并 sleep 10ms 再试。
  ...
  std::transform(ids.begin(), ids.end(), std::back_inserter(threads),
                 [&](int id) {
                   // transform 顺序遍历 ids，将返回的 thread 按序 push 到 threads
                   return std::thread(thread_func, id);
                 });

  // 等待所有线程完成（用 accumulate 触发 join，顺序遍历 threads 逐个 join，初始值 0 只是占位）
