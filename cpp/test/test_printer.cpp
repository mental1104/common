...
TEST(ContainerPrinterTest, PrintForwardList) {
  ...
  int line = __LINE__;
  mental1104::ContainerPrinter::print(flist, out);
// forward_list没有 .size() 方法
#if __cplusplus >= 202002L
  EXPECT_EQ(out.str(),
            std::format(
                "[File: test_printer.cpp, Line: {}] {{1, 2, 3, 4, 5, 6, 7}}\n",
                line));
...
TEST(ContainerPrinterTest, PrintList) {
  ...
  std::ostringstream out;
  int line = __LINE__;
  mental1104::ContainerPrinter::print(l, out);
#if __cplusplus >= 202002L
  EXPECT_EQ(out.str(), std::format("[File: test_printer.cpp, Line: {}] (size: "
                                   "7) \n{{1, 2, 3, 4, 5, 6, 7}}\n",
                                   line));
#else
  EXPECT_EQ(out.str(), "(size: 7) \n{1, 2, 3, 4, 5, 6, 7}\n");
...
TEST(ContainerPrinterTest, PrintVector) {
  ...
  std::ostringstream out;
  int line = __LINE__;
  mental1104::ContainerPrinter::print(v, out);
#if __cplusplus >= 202002L
  EXPECT_EQ(out.str(), std::format("[File: test_printer.cpp, Line: {}] (size: "
                                   "7) \n{{1, 2, 3, 4, 5, 6, 7}}\n",
                                   line));
...
TEST(ContainerPrinterTest, PrintMap) {
  ...
  std::ostringstream out;
  int line = __LINE__;
  mental1104::ContainerPrinter::print(m, out);
#if __cplusplus >= 202002L
  EXPECT_EQ(
      out.str(),
      std::format("[File: test_printer.cpp, Line: {}] (size: 3) \n{{\n    "
                  "\"a\": \"1\",\n    \"b\": \"2\",\n    \"c\": \"3\"\n}}\n",
                  line));
#else
  EXPECT_EQ(out.str(),
  ...
  line = __LINE__;
  mental1104::ContainerPrinter::print(nested_map, out);
#if __cplusplus >= 202002L
  EXPECT_EQ(out.str(),
            std::format("[File: test_printer.cpp, Line: {}] (size: 2) \n{{\n   "
                        " \"outer1\": {{\n        \"b\": \"2\",\n        "
                        "\"a\": \"1\"\n    }},\n    \"outer2\": {{\n        "
                        "\"d\": \"4\",\n        \"c\": \"3\"\n    }}\n}}\n",
                        line));
...
TEST(ContainerPrinterTest, PrintUnorderMap) {
  ...
  std::unordered_map<std::string, std::unordered_map<std::string, int>>
      nested_umap = {{"outer1", {{"a", 1}, {"b", 2}}},
                     {"outer2", {{"c", 3}, {"d", 4}}}};
  line = __LINE__;
  mental1104::ContainerPrinter::print(nested_umap, out);
#if __cplusplus >= 202002L
  EXPECT_EQ(out.str(),
            std::format("[File: test_printer.cpp, Line: {}] (size: 2) \n{{\n   "
                        " \"outer2\": {{\n        \"d\": \"4\",\n        "
