...
if(APPLE)
  ...
  set(_m1104_llvm_root "")
  if(CMAKE_CXX_COMPILER_ID MATCHES "Clang")
    set(_m1104_cxx_paths "")
    if(CMAKE_CXX_COMPILER)
      list(APPEND _m1104_cxx_paths "${CMAKE_CXX_COMPILER}")
      get_filename_component(_m1104_cxx_realpath "${CMAKE_CXX_COMPILER}" REALPATH)
      list(APPEND _m1104_cxx_paths "${_m1104_cxx_realpath}")
    endif()
    list(REMOVE_DUPLICATES _m1104_cxx_paths)
    foreach(_cand IN LISTS _m1104_cxx_paths)
      if(NOT _cand)
        continue()
      endif()
      get_filename_component(_m1104_cxx_bin "${_cand}" DIRECTORY)
      get_filename_component(_m1104_cxx_root "${_m1104_cxx_bin}" DIRECTORY)
      if(EXISTS "${_m1104_cxx_root}/include/c++/v1")
        set(_m1104_llvm_root "${_m1104_cxx_root}")
        break()
      endif()
    endforeach()
  endif()
  ...
  if(_m1104_llvm_root)
    set(_m1104_llvm_cxx_inc "${_m1104_llvm_root}/include/c++/v1")
    set(_m1104_llvm_cxx_lib "")
    foreach(_cand IN ITEMS
        "${_m1104_llvm_root}/lib"
        "${_m1104_llvm_root}/lib/c++"
        )
      if(EXISTS "${_cand}/libc++.dylib" OR EXISTS "${_cand}/libc++.1.dylib")
        set(_m1104_llvm_cxx_lib "${_cand}")
        break()
      endif()
    endforeach()
    if(EXISTS "${_m1104_llvm_cxx_inc}" AND _m1104_llvm_cxx_lib)
      add_compile_options(-stdlib=libc++)
      set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -stdlib=libc++")
      include_directories(SYSTEM "${_m1104_llvm_cxx_inc}")
      link_directories("${_m1104_llvm_cxx_lib}")
      set(CMAKE_EXE_LINKER_FLAGS "${CMAKE_EXE_LINKER_FLAGS} -stdlib=libc++ -Wl,-rpath,${_m1104_llvm_cxx_lib}")
      set(CMAKE_SHARED_LINKER_FLAGS "${CMAKE_SHARED_LINKER_FLAGS} -stdlib=libc++ -Wl,-rpath,${_m1104_llvm_cxx_lib}")
    endif()
  endif()
endif()
