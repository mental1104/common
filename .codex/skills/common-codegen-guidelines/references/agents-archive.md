...
- C++/MSVC: keep Windows builds warning-clean for stacktrace/log/random/C API RAII/tests; fix code or tests instead of suppressing workflow failures.
- C++ benchmark: vendored Google Benchmark is embedded with testing/install/Werror disabled so third-party warning policy does not fail macOS clang builds.
