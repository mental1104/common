...
- Keep CI and Pages conventions aligned: language workflows are reusable execution units, `ci-main.yml` routes Ready PR and `main` changes to the affected language workflows, and coverage extraction scripts under `tools/ci/` produce normalized `cov.json`.
- GitHub Actions 由目标为 `main` 的非草稿 PR、`main` push 或仓库级手动 `Main Gate` 启动。Ready PR 按 base/head 差异选择受影响语言，并运行该语言最新稳定的快速通道；`main` push 按 before/head 差异选择受影响语言并传入 `full_matrix: true`；手动 `Main Gate` 在 `main` 上展开六个语言完整矩阵。Draft PR 不得分配 runner，普通分支 push 不触发 Actions。
...
- Each language workflow exposes `full_matrix` through `workflow_call`. Keep one shared set of steps: Ready PR callers use the default fast lane, while affected `main` updates and repository-level manual validation use `full_matrix: true`. Do not create parallel `quick-*` and `full-*` workflow copies.
- `ci-main.yml` is the only GitHub Actions entrypoint. A non-draft PR targeting `main` computes changed files from the PR base/head and calls only affected language workflows in fast mode. A `main` push compares `github.event.before` with `github.sha`, calls only affected language workflows with `full_matrix: true`, and publishes Pages only when walkthrough/CI presentation inputs changed. A manual run is restricted to `main`, calls all language workflows with `full_matrix: true`, does not deploy Pages, and sends mail only when that manual gate fails.
