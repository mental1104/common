...
from devtool.commands.ops import cpp as cpp_ops, dotnet as dotnet_ops, go as go_ops, java as java_ops, python as python_ops, rust as rust_ops


@register("build")
def configure(subparsers: ArgumentParser):
    parser = subparsers.add_parser("build", help="Build project components")
    parser.add_argument("target", choices=["python", "cpp", "go", "rust", "dotnet", "java", "all"], nargs="?", default="all", help="Target to build")
...
def run(args):
    ...
    if target in ("java", "all"):
        java_ops.build(env, skip_tests=True)
