...
def _pip_mirror_args(env: Mapping[str, str]) -> list[str]:
    opts = env.get("PIP_MIRROR_OPTS", "")
...
def _install_export_layer(env: Mapping[str, str]) -> None:
    if (Path("export") / "python").is_dir():
        run(
            [env["PY_VENV_PIP"], "install", "--no-build-isolation", *_pip_mirror_args(env), "-e", "."],
            env=env,
            cwd=Path("export") / "python",
        )
...
def _install_requirements(env: Mapping[str, str]) -> None:
    ...
    try:
        run(
            [env["PY_VENV_PIP"], "install", "--no-build-isolation", *_pip_mirror_args(env), "-r", str(install_path)],
            env=env,
            cwd=PY_DIR,
        )
    finally:
...
def _build_wheel(env: Mapping[str, str]) -> None:
    ...
    run(
        [env["PY_VENV_PYTHON"], "-m", "pip", "wheel", "--no-deps", "-w", "dist", *_pip_mirror_args(env), "."],
        env=env,
        cwd=PY_DIR,
    )
...
def test(
    ...
    *,
    ...
    try:
        ...
        run(
            [venv_env["PY_VENV_PIP"], "install", *_pip_mirror_args(venv_env), "pytest-benchmark"],
            env=venv_env,
        )
