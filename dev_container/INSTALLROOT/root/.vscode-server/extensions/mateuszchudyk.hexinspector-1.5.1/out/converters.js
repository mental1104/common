...
exports.toBitSet = exports.toSize = exports.toAscii = exports.toFloat64 = exports.toFloat32 = exports.toFloat16 = exports.toHexadecimal = exports.toBinary = exports.toDecimalSigned = exports.toDecimalUnsigned = exports.fromHexadecimal = exports.fromDecimal = exports.fromBinary = void 0;
const utils = require("./utils");
//
// From X to bytes.
//
function fromBinary(str, isLittleEndian) {
    if (!str) {
        return undefined;
    }
...
function fromDecimal(str, isLittleEndian) {
    if (!str) {
        return undefined;
    }
...
function fromHexadecimal(str, isLittleEndian) {
    if (!str) {
        return undefined;
    }
    str = utils.reverseString(str);
...
exports.fromHexadecimal = fromHexadecimal;
//
// From bytes to X.
//
function toDecimalUnsigned(bytes) {
    if (bytes.length == 0) {
        return '';
    }
...
exports.toDecimalUnsigned = toDecimalUnsigned;
function toDecimalSigned(bytes) {
    if (bytes.length == 0) {
        return '';
    }
...
exports.toDecimalSigned = toDecimalSigned;
...
exports.toFloat16 = toFloat16;
function toFloat32(bytes) {
    if (bytes.length > 4 || bytes.length == 0) {
        return '';
    }
    var sign = bytes.length == 4 ? bytes[3] >> 7 : 0;
    var exponent = ((bytes.length == 4 ? bytes[3] & 0x7f : 0) << 1) +
        (bytes.length >= 3 ? bytes[2] >> 7 : 0);
    var significand = ((bytes.length >= 3 ? bytes[2] & 0x7f : 0) << 16) +
...
function toFloat64(bytes) {
    ...
    var exponent = ((bytes.length == 8 ? bytes[7] & 0x7f : 0) << 4) +
        (bytes.length >= 7 ? bytes[6] >> 4 : 0);
    var significand_hi = ((bytes.length >= 7 ? bytes[6] & 0x0f : 0) << 16) +
        ((bytes.length >= 6 ? bytes[5] : 0) << 8) +
        (bytes.length >= 5 ? bytes[4] : 0);
    var significand_low = (bytes.length >= 4 ? bytes[3] : 0) +
