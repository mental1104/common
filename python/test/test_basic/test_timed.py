...
from mental1104 import async_timed, get_current_time, timed  # 替换为实际模块名
...
class TestTimeHelper:
    ...
    def test_get_current_time_with_parameters(self, fmt, zone, expected_format):
        """
        【场景背景】调用方可自定义格式和时区。
        【步骤输入】使用参数化组合多种 format/zone。
        【期望输出】返回值与 datetime.now 对应格式完全一致。
        """
        current_time = get_current_time(fmt=fmt, zone=zone)
