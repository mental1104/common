...
def collect_public_names(file_path: Path) -> List[str]:
    ...
    with file_path.open("r", encoding="utf-8") as f:
        try:
            ...
            # 多个文件需多次调用并传入各自源码
            tree = ast.parse(f.read(), filename=str(file_path))  # 解析源码为 AST，filename 便于报错定位，tree 为 ast.Module 根节点
        except SyntaxError:
            return []
    ...
    for node in tree.body:  # 遍历模块顶层语句，node 是 ast.AST 子类实例，挑选公开的类/函数/变量名
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):  # 判断顶层同步/异步函数或类
        ...
        if isinstance(node, ast.Import):  # 检测 import xxx 语句
            for alias in node.names:
                if alias.name == PKG or alias.name.startswith(PKG + "."):
                    return True
        elif isinstance(node, ast.ImportFrom):  # 检测 from xxx import ... 语句
...
def walk_modules(base_dir: Path) -> List[ModuleInfo]:
    ...
    for dirpath, dirnames, filenames in os.walk(base_dir):  # os.walk 返回迭代器，逐层递归遍历目录，遍历完覆盖全部子树但不会一次性列出全部文件
        ...
        for filename in py_files:  # 内层循环处理当前目录的每个 .py 文件，外层遍历目录、内层遍历文件形成双重循环
            ...
            rel = file_path.relative_to(base_dir).with_suffix("")  # relative_to 计算相对 base_dir 的子路径，去掉后缀用于模块名组装
    ...
    for mi in mods:
        for n in mi.names:
            if n not in export_map:
                ...
                dups.setdefault(n, [])  # 记录重名模块列表，便于生成提示注释，export_map 已定胜者但 dups 保留所有提供者
...
def generate_init(mods: List[ModuleInfo]) -> str:
    ...
    lines = [
        ...
        *dup_lines,
        "",
        "# ---- Direct imports (safe modules; no mental1104 top-level imports) ----",  # 安全模块已上面直接导入，访问时不会触发 __getattr__
        *direct_import_lines,
        "",
...
def main():
    ...
    INIT_FILE.write_text(content, encoding="utf-8")  # Path.write_text 写入文本文件，自动打开/覆盖目标，指定编码
