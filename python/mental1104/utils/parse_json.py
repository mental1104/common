...
try:
    from mental1104_export_layer import (  # type: ignore
        get_active_strategy_name as _get_strategy,
        parse_json as _cpp_parse_json,
    )
...
__all__ = ["JsonParserType", "JsonUtil", "dump_json", "load_json"]
...
class JsonUtil:
    ...
    __parsers: ClassVar[dict[str, Callable[[str | bytes], Any]]] = {"json": json.loads}
    __modules: ClassVar[dict[str, Any]] = {"json": json}
    ...
    __parser_names: ClassVar[tuple[str, ...]] = tuple(__parsers.keys())
...
@singledispatch
def _parse_dispatch(value: object, _parser: str) -> Any:
    ...
    *,
    ...
    if parser_name == "orjson":
        try:
            ...
            if indent is not None and int(indent) != 0 and not (int(indent) == 2 and has_indent2):
