...
class TaskExecutionStrategy(ABC):
    ...
    @abstractmethod
    async def execute(self, _loop, tasks: List[Awaitable[_T]]) -> List[_T]:
...
class CoroutinePool:
    ...
    async def run_task_batch(
        self, partial_funcs: List[functools.partial], strategy: TaskExecutionStrategy
    ) -> List[_T]:
        tasks: List[Awaitable[_T]] = [self.worker(pf) for pf in partial_funcs]
        return await strategy.execute(self.loop, tasks)

    def run(
        self, coros: List[functools.partial], strategy: TaskExecutionStrategy | None = None
    ) -> List[_T]:
        if strategy is None:
...
def _shutdown_executor(ex: Executor) -> None:
    try:
...
class _BaseExecutorCoroutinePool(CoroutinePool, ABC):
    ...
    def _ensure_executors(self) -> List[Executor]:
        ...
        self._executors = [self._create_executor(i, per_shard) for i in range(self.n_shards)]
        ...
        try:
            ...
            for pf in partial_funcs:
                ...
                def _release(_fut, shard_idx=shard, self_ref=self):
                    self_ref._shard_loads[shard_idx] = max(0, self_ref._shard_loads[shard_idx] - 1)
