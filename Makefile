...
setup: ## 默认：安装依赖并执行自定义步骤
	$(SUDO_MSG)
	$(SHELL) -lc 'set -e; \
	py_install(){ $(SUDO) $(PIP3) install python/ --upgrade; }; \
	py_run_init(){ if [[ -f python/generate_init.py ]]; then echo "[info] 执行 python/generate_init.py …"; $(SUDO) $(PYTHON) python/generate_init.py; else echo "[info] 未找到 python/generate_init.py，跳过。"; fi; }; \
	py_install; \
	py_run_init; \
	echo "[ok] setup 完成。"'
...
install: ## 安装本地包（非editable）
	$(SUDO_MSG)
	$(SHELL) -lc 'set -e; $(SUDO) $(PIP3) install python/ --upgrade; echo "[ok] 安装完成。"'
